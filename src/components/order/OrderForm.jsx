...
export default function OrderForm() {
  ...
  const isScheduled = order.scheduledDate ? new Date(order.scheduledDate) > new Date() : false;
      ...
      try {
        ...
        if (response.data.result.status === 'success') {
          ...
          setCustomer({
            ...
            blacklist: !!data.customer?.blacklist,
          });
  ...
  const handleSubmit = async () => {
    ...
    const scheduledForLater = new Date(order.scheduledDate) > new Date();
    const orderState = isDraftOrder ? 'not_prepared' : (scheduledForLater ? 'draft' : 'not_prepared');
    const result = await saveOrder(orderState);
    if (result) {
      const stateText = orderState === 'draft' ? `scheduled for ${order.scheduledDate.replace('T', ' ')}` : 'submitted successfully!';
  ...
  const applyCustomerData = (customerData, location) => {
    ...
    setCustomer({
      ...
      blacklist: !!customerData.blacklist,
    });
            ...
            <Box>
              ...
              <Box sx={{
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 5, flexWrap: 'wrap', gap: 2 }}>
                  ...
                  {/* already paid online + blacklist banner */}
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    {customer.blacklist && (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 1.2, borderRadius: R.pill, border: `1.5px solid ${alpha(C.red, 0.45)}`, backgroundColor: alpha(C.red, 0.07) }}>
                        <BlockIcon sx={{ fontSize: 18, color: C.red }} />
                        <Box>
                          <Typography sx={{ fontFamily: FONT, fontWeight: 700, fontSize: '13px', color: C.red, lineHeight: 1.2 }}>Blacklisted Customer</Typography>
                          <Typography sx={{ fontFamily: FONT, fontSize: '11px', color: alpha(C.red, 0.85) }}>Collect payment before delivery</Typography>
                        </Box>
                      </Box>
                    )}
